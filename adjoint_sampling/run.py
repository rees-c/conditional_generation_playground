...
def main():
    ...
    energy_fn: nn.Module = get_energy_function(device)
    ...
    target_dist = torch.exp(-energy_fn(grid) / energy_temperature) #energy_fn.gmm.log_prob(grid).exp()
    ...
    plt.show()
    # plt.savefig("adjoint_sampling_epoch0.png")
    # plt.close()
    ...
    samples = samples.cpu().numpy()
    plt.plot(samples[:, 0], samples[:, 1], "rx", alpha=0.5)
    plt.savefig(f"adjoint_sampling_epoch{epoch}.png")
    plt.close()
