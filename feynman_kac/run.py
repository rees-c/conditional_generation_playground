...
class DiffusionModel(nn.Module):
    ...
    def predict_intermediate_reward(self, x, t) -> Tensor:
        """Predict log \mathbb{E}_{p(x_0 | x_t)} [exp(r(x_0)]"""
        batch_size: int = x.shape[0]
        if isinstance(t, Tensor):
            float_t = t.float()
        elif isinstance(t, int):
        ...
        with torch.no_grad():
            # Don't let the reward model mess up the diffusion model
            t_emb = self.time_emb(float_t).expand(batch_size, -1)
            x_emb = self.pos_emb(x)

        prediction = self.reward_model(torch.cat([x_emb, t_emb], dim=-1))
        return prediction

    def compute_intermediate_reward_loss(self, x0: Tensor, reward: Tensor):
        """
        Assumes model was pretrained on x0 with score matching so that we can
        approximate p_\theta(xt|x0) with q(xt|x0)
        """
        device = x0.device

        # ------------ Sample xt
        ts = self.noise_scheduler.uniform_sample_timestep(
            batch_size=x0.shape[0], device=device
        )
        # (batch_size,)
        sigmas = self.noise_scheduler.sigmas[ts].view(*x0.shape)
        # (batch_size, spatial_dim)
        noise = torch.randn_like(x0)
        with torch.no_grad():
            xt = (x0 + sigmas * noise) % 1.0

        pred_intermediate_reward = self.predict_intermediate_reward(xt, ts)
        loss = F.mse_loss(pred_intermediate_reward, reward)
        return loss
    ...
    @torch.no_grad()
    def sample_with_fk_steering(
        self,
        batch_size: int,
        reward_fn: Callable,
        Lambda: float = 1.0,
        num_particles: int = 4,
        resampling_interval: int = 3,  # >1 encourages exploration/increases efficiency
        device: torch.device = "cpu",
    ):
        """FKS with difference potentials and learned intermediate rewards"""
        _training = self.training
        self.training = False

        num_timesteps = self.noise_scheduler.num_timesteps
        t = num_timesteps-1
        x_t_plus1 = torch.rand(
            batch_size * num_particles, self.spatial_dim, device=device
        )
        reward_t_plus1 = reward_fn(x_t_plus1)
        potentials = torch.ones_like(x_t_plus1)  # difference potential has GT=1
        _arange = torch.arange(batch_size, device=device)[:, None]
        for step in range(num_timesteps-1):
            # -- Resample
            if step % resampling_interval == 0:
                potentials = potentials.view(batch_size, num_particles)

                # - Adaptive resampling to preserve particle diversity: only
                # resample if effective sample size is greater than k/2.
                # Implement with boolean masking.
                normalized_potentials = (
                    potentials / potentials.sum(dim=-1, keepdim=True)
                )  # (batch_size, num_particles)
                effective_sample_size = 1 / normalized_potentials.pow(2).sum(dim=-1)
                # (batch_size,)
                should_resample_mask = (
                    effective_sample_size > num_particles / 2
                ).repeat_interleave(num_particles)
                # (batch_size * num_particles,)

                # - resample
                sampled_particle_idxs = torch.multinomial(
                    input=potentials, num_samples=num_particles, replacement=True
                )  # (batch_size, num_particles)
                sampled_particle_idxs = (sampled_particle_idxs + _arange).view(-1)
                # (batch_size * num_particles,)
                x_t_plus1[should_resample_mask] = x_t_plus1[
                    sampled_particle_idxs[should_resample_mask]
                ]
            ...
            # x_t <- x_t + sqrt(sigma_t+1 ** 2 - sigma_t ** 2) * z
            sigma_norm_t_plus_1 = self.noise_scheduler.sigmas_norm[t+1]
            float_t = torch.tensor([t+1], dtype=torch.float, device=device)
            predicted_score = torch.sqrt(sigma_norm_t_plus_1) * self(x_t_plus1, float_t)
            sigma_sq_diff = (
                self.noise_scheduler.sigmas[t+1] ** 2
                - self.noise_scheduler.sigmas[t] ** 2
            )  # (,)
            ...
            x_t = (mean + std * noise) % 1.0

            # -- Reweight with difference potential
            t = t-1
            if step == num_timesteps-2:
                reward_t = reward_fn(x_t)
            else:
                reward_t = self.predict_intermediate_reward(x_t, t)
            potentials = torch.exp(Lambda * (reward_t - reward_t_plus1))
            reward_t_plus1 = reward_t
            x_t_plus1 = x_t
...
def plot_current_policy(
    model, reward_fn, device, method: str, epoch: int, use_fk_steering: bool = False
):
    if use_fk_steering:
        samples = model.sample_with_fk_steering(
            100, reward_fn, num_particles=4, device=device
        )
    else:
...
def get_pretraining_data(spatial_dim) -> Tensor:
...
def pretrain():
    ...
    data = get_pretraining_data(spatial_dim)
    # (dataset_size, spatial_dim)
...
def train_intermediate_reward_model(model: DiffusionModel, reward_fn: Callable):
    lr = 1e-3
    num_epochs = 1001
    batch_size = 32
    log_freq = 100

    dataset = get_pretraining_data(spatial_dim)
    rewards = reward_fn(dataset)
    dataset_size = dataset.shape[0]

    optimizer = torch.optim.AdamW(model.reward_model.parameters(), lr=lr)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, min_lr=1e-5)
    meter = AverageMeter()
    val_meter = AverageMeter()
    for epoch in range(num_epochs):
        shuffle_idxs = torch.randperm(dataset_size, device=device)
        shuffled_data = dataset[shuffle_idxs]
        for batch_idx in range(math.ceil(dataset_size / batch_size)):
            ...
            loss = model.compute_intermediate_reward_loss(batch, batch_rewards)
            loss.backward()
            optimizer.step()
            optimizer.zero_grad()
        ...
        val_meter.reset()
        if epoch % log_freq == 0:
            print(f"Epoch {epoch}: {meter.avg:0.4f}")
            meter.reset()

    # Plot Pearson correlation coefficient for predicted intermediate rewards
    # vs MC estimate todo
...
def run(method: str, use_pretrained_model: bool = True):
    assert method in ["dpok", "fks"], f"{method} not implemented."
    device = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    plot_current_policy(model, reward_fn, device, method, 0, method == "fks")
    if method == "fks":
        # Train intermediate reward
        # todo: pre-training diffusion on dataset is required if you want to
        #  train intermediate rewards on that data. Otherwise, need to forward
        #  sample trajectories from the model and label each trajectories with
        #  its final reward, then use those trajectories/rewards as data.
        train_intermediate_reward_model(model, reward_fn)
    elif method == "dpok":
        run_dpok(
            model, num_iterations, normalize_advantages,
    ...
    # plot samples against reward
    plot_current_policy(
        model, reward_fn, device, method, num_iterations, method == "fks"
    )
...
if __name__ == "__main__":
    ...
    method = "fks"  # ["dpok", "fks"]
    if use_pretrained_model or method == "fks":
        pretrain()
